...
#           $ popd
#       2. Run the program
#           $ # Note that there are more options.  Run `create_systemd_scope.py -h` for more info
#           $ sudo PYTHONPATH=/path/to/libcgroup.cpython*.so \
#             ./samples/python/create_systemd_scope.py \
#             --slice <yourslicename> --scope <yourscopename>
#
#           $ export PYTHONPATH=$PYTHONPATH:src/python/build/
...
def move_pids_to_tmp_cgroup():
    ...
    pids = cg.get_procs()
    for pid in pids:
        Cgroup.move_proc(pid, TMP_CGNAME)
