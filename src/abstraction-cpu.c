...
static int get_period(struct cgroup_controller * const cgc,
		      char ** const period)
{
	return read_setting(cgc->cgroup->name, "cpu", CFS_PERIOD_US,
			period);
}
	...
	void *in_dflt, void *out_dflt)
{
	...
	if (strlen(in_value) == 0) {
		...
		snprintf(max_line, LL_MAX, "%s %s", MAX, quota);
		ret = cgroup_add_value_string(dst_cgc, out_setting, max_line);
		if (ret)
			goto out;
	}
	...
	void *in_dflt, void *out_dflt)
{
	...
	token = strtok_r(in_value, " ", &saveptr);
