...
def test_cpuset(config):
    ...
    CgroupCli.set(config, cgname=SCOPE, setting='AllowedCPUs', value='0,1,3,4,5,7-11,12',
...
def test(config):
    ...
    for entry in TABLE:
        CgroupCli.set(config, cgname=SCOPE, setting=entry[0], value=entry[1], systemd_property=True)
