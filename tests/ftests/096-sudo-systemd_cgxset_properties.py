...
#
# Set properties on systemd-managed cgroups
#
...
def test_cpuset(config):
    ...
    CgroupCli.xset(config, cgname=SCOPE, setting='AllowedCPUs', value='0,1,3,4,5,7-11,12',
                   systemd_property=True)
...
def test(config):
    ...
    # TODO - add support to cgset for systemd booleans and then use cgset to set these values
    ...
    for entry in TABLE:
        CgroupCli.xset(config, cgname=SCOPE, setting=entry[0], value=entry[1], systemd_property=True)
