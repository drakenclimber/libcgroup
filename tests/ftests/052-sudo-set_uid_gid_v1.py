...
#
# Test to set the uid/gid on a cgroup using the python bindings
#
...
def test(config):
    ...
    if tasks_gid != TASKS_GID:
        result = consts.TEST_FAILED
        cause = 'Expected tasks group to be {} but it\'s {}'.format(TASKS_GID, tasks_gid)
    ...
    if ctrl_uid != CTRL_UID:
        result = consts.TEST_FAILED
        cause = 'Expected cgroup.procs owner to be {} but it\'s {}'.format(CTRL_UID, ctrl_uid)

    ctrl_gid = utils.get_file_owner_gid(config, ctrl_path)
    if ctrl_gid != CTRL_GID:
        result = consts.TEST_FAILED
        cause = 'Expected cgroup.procs group to be {} but it\'s {}'.format(CTRL_GID, ctrl_gid)
